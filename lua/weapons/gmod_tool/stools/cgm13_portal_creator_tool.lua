TOOL.Category = "ARC Tools"
TOOL.Name = "#Tool.cgm13_portal_creator_tool.name"
...
function TOOL:GetPlacementPosition(tr)
	...
	--
	return (tr.HitPos + tr.HitNormal * self:GetOwner():GetInfoNum("portal_size_z", 1) * 10), rotatedAng
end
...
if ( CLIENT ) then
	...
	language.Add("Tool.cgm13_portal_creator_tool.name", "Portal Creator")
	language.Add("Tool.cgm13_portal_creator_tool.desc", "Creates and links portals")
	language.Add("Tool.cgm13_portal_creator_tool.left", "Left Click: Create portal")
	language.Add("Tool.cgm13_portal_creator_tool.right1", "Right Click: Start linking a portal")
	language.Add("Tool.cgm13_portal_creator_tool.right2", "Right Click: Create link to another portal")

	-- yoink! smiley :)
	local xVar = CreateClientConVar("portal_size_x", "1", false, true, "Sets the size of the portal along the X axis", 0.01, 10)
	local yVar = CreateClientConVar("portal_size_y", "1", false, true, "Sets the size of the portal along the Y axis", 0.01, 10)
	local zVar = CreateClientConVar("portal_size_z", "1", false, true, "Sets the size of the portal along the Z axis", 0.01, 10)

	function TOOL.BuildCPanel(panel)
		panel:AddControl("label", {
			text = "Creates and links portals. The portals code is derived from 'Seamless Portals', created by 'Mee'.",
		})
		panel:NumSlider("Portal Size X", "portal_size_x", 0.05, 10, 2)
		panel:NumSlider("Portal Size Y", "portal_size_y", 0.05, 10, 2)
		panel:NumSlider("Portal Size Z", "portal_size_z", 0.05, 10, 2)
	end
			...
			if self:GetStage() == 2 then
				...
				if IsValid(target) then
					...
					if tr.Hit then
						local ent = tr.Entity
						if IsValid(ent) then
							if ent:GetClass() == "cgm13_portal" then
...
elseif ( SERVER ) then
	...
	function TOOL:LeftClick(trace)
		...
		local ent = ents.Create("cgm13_portal")
		ent:SetPos(pos)
		...
		local sizez = self:GetOwner():GetInfoNum("portal_size_z", 1)
	...
	function TOOL:GetTarget(trace)
		...
		if ent:GetClass() ~= "cgm13_portal" then return NULL end
		if CPPI then
			if not ent:CPPICanTool(self:GetOwner(), "cgm13_portal_creator_tool") then return NULL end
		end
		return ent
	end
